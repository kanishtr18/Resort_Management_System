...
@RestController
@RequestMapping("/api/billing/ledger")  // Fix: was /api/ledger
public class AccountLedgerController {
    ...
    @PostMapping
    @PreAuthorize("hasAuthority('ADMIN')")
    public ResponseEntity<AccountLedgerResponse> create(
            @Valid @RequestBody AccountLedgerRequest request) {
        AccountLedger ledger = BillingMapper.toEntity(request);
        AccountLedger created = service.save(ledger);
        return ResponseEntity.status(HttpStatus.CREATED).body(BillingMapper.toResponse(created));
    }
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAuthority('ADMIN')")
    public ResponseEntity<AccountLedgerResponse> update(
            @PathVariable UUID id,
            @Valid @RequestBody AccountLedgerRequest request) {
        // Fix: use isEmpty() instead of !isPresent()
        if (service.findById(id).isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        AccountLedger ledger = BillingMapper.toEntity(request);
        ledger.setId(id);
        return ResponseEntity.ok(BillingMapper.toResponse(service.save(ledger)));
    }
